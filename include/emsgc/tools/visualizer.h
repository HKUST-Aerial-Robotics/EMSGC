...
namespace emsgc
{
  ...
  namespace tools
  {
    ...
    * number of motions. */
        ...
        )
    {
      ...
      for(;it != mVols.end(); it++)
      {
        ...
        for(auto& site:vol.seg_)
        {
          ...
          cv::Scalar color;
          //      if(intensity < 0.1)
          //        color = cv::Scalar(255.f,255.f,255.f);
          //      else
          color = cv::Scalar(intensity * b[index], intensity * g[index], intensity * r[index]);

          //draw the point
          cv::Point point;
          point.x = x;
          point.y = y;
          cv::circle(output, point, 1, color, cv::FILLED);
        }
      }
    }
    ...
    /* Draw labeled IWE. */
    inline void drawColorIWE_with_EventClustersGMM_HSV(
        std::map<int, EventClusterGMM>& mEvtClustersGMM,
        ros::Time& t_ref,
        OptionsWarp& opts_warp,
        size_t width,
        ...
        )
    {
      ...
      // Compute each IWE (cluster)
      const size_t numLabels = mEvtClustersGMM.size();
      std::vector<cv::Mat> vIWE(numLabels);
      cv::Mat iwe_sum = cv::Mat::zeros(img_size, CV_32FC1);
      size_t iCount = 0;
      for(auto it = mEvtClustersGMM.begin(); it != mEvtClustersGMM.end(); it++, iCount++)
      {
        auto & ec = it->second;
        std::vector<UndistortedEvent> vUndistEvents;
        vUndistEvents.reserve(ec.dqEvtSites_.size());
        ...
        computeImageOfWarpedEvents_GMM_undistortion(ec.gmm_, vUndistEvents,
          t_ref.toSec(), img_size, &vIWE.at(iCount), opts_warp_display);

        iwe_sum += vIWE.at(iCount);
      }
      ...
      cv::minMaxLoc(iwe_sum, nullptr, &max_num_wevents_at_pixel);
      cv::Mat output_hsv = cv::Mat::zeros(img_size, CV_32FC3);
      for(size_t y = 0; y < height; y++)
      {
        for(size_t x = 0; x < width; x++)
        {
          const float num_wevents_at_pixel = iwe_sum.at<float>(y,x);
      ...
      // Convert color spaces and type.
      output_hsv.convertTo(output_hsv, CV_8UC3);
